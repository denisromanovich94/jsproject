export class TaskList {
    ...
    constructor (store) {
        ...
        this.#tasksListDone = document.querySelector(".done-tasks");
        this.#taskInput = document.querySelector(".input-text");
    ...
    renderTasks() {
        ...
        tasks.forEach((task) => {
          ...
          doneHtml += `
            <div class="task__item done-tasks">
              <p class="task__text done-text">${task.text}</p>
            </div>`;
        });
        this.#tasksListDone.innerHTML = doneHtml;
      }
      ...
      async handleAddTask(event) {
        ...
        try {
          await this.#store.addTask(text);
          this.renderTasks();
        } catch (error) {
          this.#errorMessage.innerHTML = "Ошибка: Не удалось добавить задачу";
      ...
      async handleDeleteTask(event) {
        ...
        if (deleteButton) {
          ...
          const taskId = parseInt(parentNode.id.replace("task-", ""));
          try {
      ...
      async handleDoneTask(event) {
        ...
        if (doneButton) {
          ...
          try {
            await this.#store.markTaskAsDone(taskId, taskText);
            this.renderTasks();
          } catch (error) {
            this.#errorMessage.innerHTML = "Ошибка: Не удалось отметить задачу как выполненную";
            setTimeout(() => (this.#errorMessage.innerHTML = ""), 5000);
          }
        }
      }
      ...
      init() {
        ...
        form.addEventListener("submit", (event) => this.handleAddTask(event));
        this.#tasksList.addEventListener("click", (event) => this.handleDeleteTask(event));
        this.#tasksList.addEventListener("click", (event) => this.handleDoneTask(event));
      }
    }
